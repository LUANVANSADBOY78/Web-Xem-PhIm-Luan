...
function Header() {
  ...
  return <header className="site-header"><Link className="brand" to="/" aria-label={settings.name}>{settings.logo && <img src={safeMedia(settings.logo)} alt="" style={{maxWidth:48,maxHeight:40,marginRight:8}} />}<span className="brand-name">{settings.name}</span></Link><button className="mobile-toggle" onClick={() => setMobile(!mobile)} aria-label="Mở menu" aria-expanded={mobile}>☰</button>
...
function Footer() {
  ...
  return <footer><div className="container footer-grid"><div><p><Link to="/">Xem phim online</Link> với phụ đề Tiếng Việt, Thuyết Minh và Lồng Tiếng. Khám phá phim chiếu rạp, phim bộ Trung Quốc, Hàn Quốc và nhiều thể loại khác.</p><p>{settings.description}</p><p>{settings.footer}</p>{settings.contact_email && <a href={'mailto:'+settings.contact_email}>{settings.contact_email}</a>}</div><div><h3>Trợ giúp</h3><Link to="/terms">Điều khoản chung</Link><Link to="/privacy">Chính sách riêng tư</Link><a href="/sitemap.xml">Sitemap</a><a href="/movies.xml">RSS Feed</a></div><div><h3>About</h3><Link to="/">{settings.name}</Link><Link to="/about">Giới thiệu</Link><Link to="/faq">FAQ</Link><Link to="/contact">Liên hệ</Link><Link to="/admin">Quản trị</Link></div></div></footer>;
}
function Sidebar() {
  const { movies } = useApp();
...
function Detail({ watching = false }) {
  ...
  return <main className="page container"><div className="breadcrumbs"><Link to="/">Trang chủ</Link> › <Link to={movieUrl(m)}>{m.name}</Link>{watching && ' › Xem phim'}</div><div className="columns"><div className="primary">{watching ? <><h1>{m.name}</h1><p className="muted">{current ? `${server} · Tập ${current.name}` : 'Chưa có tập phim'}</p><Video key={current?.slug} movie={m} episode={current} /><div className="server-selector"><label>Máy chủ phát<select value={server || ''} onChange={e => watch(episodes.find(ep => ep.server === e.target.value && ep.name === current?.name) || episodes.find(ep => ep.server === e.target.value))}>{[...new Set(episodes.map(ep => ep.server))].map(s => <option key={s}>{s}</option>)}</select></label></div><div className="player-actions"><button disabled={index <= 0} onClick={() => watch(serverEpisodes[index - 1])}>← Tập trước</button><button disabled={index < 0 || index >= serverEpisodes.length - 1} onClick={() => watch(serverEpisodes[index + 1])}>Tập tiếp →</button><Link to={'/contact?phim=' + slug}>⚑ Báo lỗi</Link><Link to={movieUrl(m)}>Thông tin phim</Link></div></> : <section className="detail-hero"><div className="detail-backdrop" style={{ backgroundImage: `url("${safeMedia(m.poster_url)}")` }} /><div className="detail-poster"><Poster movie={m} /><button className="primary-button" onClick={() => watch(current)}>▶ Xem phim</button></div><div className="detail-info"><h1>{m.name}</h1><h2>{m.origin_name} ({m.publish_year})</h2><p className="detail-tags"><span>{m.quality}</span><span>{m.language}</span><span>{m.episode_current}</span></p><p>Trạng thái: <strong>{m.status === 'completed' ? 'Hoàn tất' : m.status === 'trailer' ? 'Trailer' : 'Đang chiếu'}</strong></p><p>Thời lượng: {m.episode_time || 'Chưa cập nhật'}</p><p>Số tập: {m.episode_total || 'Chưa cập nhật'}</p><p>Quốc gia: {m.regions?.map(c => <Link key={c.slug} to={'/quoc-gia/' + c.slug}>{c.name} </Link>)}</p><p>Thể loại: {m.categories?.map(c => <Link key={c.slug} to={'/the-loai/' + c.slug}>{c.name} · </Link>)}</p><p><span className="gold">★ {m.rating_star || 'Chưa có điểm'}</span> · {number(m.view_total)} lượt xem</p><div className="detail-actions"><button onClick={() => profile('favorites')}>{user?.favorites?.includes(slug) ? '♥ Đã yêu thích' : '♡ Yêu thích'}</button><button onClick={() => profile('following')}>{user?.following?.includes(slug) ? '✓ Đang theo dõi' : '＋ Theo dõi'}</button></div></div></section>}
    <section className="panel"><h2>Danh sách tập phim</h2><p className="muted">{episodes.length} nguồn tập hiện có · Tổng số tập công bố: {m.episode_total || 'Chưa cập nhật'}</p>{[...new Set(episodes.map(e => e.server))].map(s => <div className="server-group" key={s}><h3>▣ {s}</h3><div className="episodes">{episodes.filter(e => e.server === s).sort(episodeOrder).map(e => <button className={watching && current?.slug === e.slug ? 'active' : ''} key={e.id || e.slug} onClick={() => watch(e)}>{e.name}{e.is_new && <small> · Mới</small>}</button>)}</div></div>)}{!episodes.length && <p>Chưa có tập phim.</p>}</section><section className="panel"><h2>Nội dung phim</h2><p>{m.description || m.content?.replace(/<[^>]+>/g, '') || 'Nội dung giới thiệu chưa được cập nhật.'}</p>{m.actor && <p>Diễn viên: {m.actor}</p>}{m.director && <p>Đạo diễn: {m.director}</p>}{m.trailer && <a className="orange" href={safeMedia(m.trailer)} target="_blank" rel="noreferrer">Xem trailer ↗</a>}</section><Community movie={m} /><section className="home-section"><h2>Có thể bạn muốn xem</h2><div className="poster-grid">{movies.filter(x => x.slug !== slug && x.categories?.some(c => m.categories?.some(t => t.slug === c.slug))).slice(0, 5).map(x => <Card movie={x} key={x.slug} />)}</div></section></div><Sidebar /></div></main>;
}
function Auth({ adminOnly = false }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      const user = await api('/auth/' + mode, 'POST', body);
      if (adminOnly && !['admin','staff'].includes(user.role)) { await api('/auth/logout','POST'); setUser(null); throw new Error('Tài khoản này không có quyền quản trị.'); }
      setUser(user); await refresh(); history.push(['admin','staff'].includes(user.role) ? '/admin' : '/tai-khoan');
    } catch (e) { notify(e.message); } finally { setBusy(false); }
  }
  return <main className="page container"><form className="auth panel" onSubmit={submit}>
    <h1>{mode === 'login' ? (adminOnly ? 'Đăng nhập quản trị' : 'Đăng nhập') : mode === 'setup' ? 'Thiết lập quản trị viên' : 'Tạo tài khoản'}</h1>
    <p className="muted">{adminOnly ? 'ADMIN MOVIE · Dành cho Admin và Staff' : 'Lưu phim yêu thích, theo dõi và tiếp tục xem.'}</p>
    {mode !== 'login' && <label>Tên hiển thị<input name="name" required maxLength={80} autoComplete="name" /></label>}
    <label>Email<input name="email" type="email" required autoComplete="email" /></label>
    <label>Mật khẩu<input name="password" type="password" required minLength={8} maxLength={200} autoComplete={mode === 'login' ? 'current-password' : 'new-password'} /></label>
    {mode === 'setup' && setupRequiresToken && <label>Mã thiết lập Admin<input name="setupToken" type="password" required autoComplete="off" /><small>Lấy ADMIN_SETUP_TOKEN trong mục Environment của dịch vụ Render.</small></label>}
    <button disabled={busy} className="primary-button">{busy ? 'Đang xử lý...' : mode === 'login' ? 'Đăng nhập' : 'Tạo tài khoản'}</button>
    {!adminOnly && (mode !== 'login' || settings.registration_enabled !== false) && <button type="button" onClick={() => setMode(mode === 'login' ? 'register' : 'login')}>{mode === 'login' ? 'Chưa có tài khoản? Đăng ký' : 'Đã có tài khoản? Đăng nhập'}</button>}
    {needsSetup && <button type="button" onClick={() => setMode(mode === 'setup' ? 'login' : 'setup')}>{mode === 'setup' ? 'Về đăng nhập' : 'Thiết lập tài khoản Admin đầu tiên'}</button>}
    {adminOnly && <Link to="/">← Về website</Link>}
  </form></main>;
}
function Account() {
