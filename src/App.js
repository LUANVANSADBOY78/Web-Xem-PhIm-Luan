...
function Auth() {
  const { setUser, needsSetup, setupRequiresToken, refresh, notify } = useApp();
...
export default function App() {
  ...
  async function refresh() { const [data, session] = await Promise.all([api('/catalog'), api('/session')]); setCatalog(data); setUser(session.user); setNeedsSetup(session.needsSetup); setSetupRequiresToken(!!session.setupRequiresToken); setError(''); }
